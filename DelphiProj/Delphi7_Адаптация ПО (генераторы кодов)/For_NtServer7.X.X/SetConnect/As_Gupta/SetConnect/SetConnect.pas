...
const
     ...
     SQLMonitorSubKey: string = 'SOFTWARE\CENTURA\SQLBase\1';
     SQLMonitorSubKeyName: string = 'ServerType';
type
...
type
  ...
  private
    ...
    procedure UpdateNumUser(MyNumUser: string);
  public
    { Public declarations }
  end;
var
  ...
  FlagBufferActive: boolean = False;
...
function RegSetValueText(My_PHKEY:integer; MyNeedSubKey, MyNeedSubKeyName, MySetingValue: String): boolean;

implementation

{$R *.DFM}
procedure TForm1.UpdateNumUser(MyNumUser: string);
begin
      if IsDigit(MyNumUser) then
       begin
        if StrToInt(MyNumUser)<>0 then
          Form1.Label4.Caption:= GetNumUser
        else
          Form1.Label4.Caption:= 'Not Limited';
       end
      else
...
Result:= false;
  ...
  try
    ...
    DataSize := length(MySetingValue);
    StrPCopy(DataBuffer, MySetingValue);
    if RegSetValueEx(phkResult, PChar(MyNeedSubKeyName), 0, REG_SZ,
                              DataBuffer,
                              DataSize)=0 then
    Result:= TRue;
  finally
...
//-------- Set ServerName in reestr for SQLBase monitor
if Form1.CheckBox2.Checked then
 begin
  TmpString:= 'SQLBase Server ';
  case MyNumUser of
   0:  TmpString:= TmpString+'Not Limited)';
   1:  TmpString:= TmpString+ IntToStr(MyNumUser)+'-User';
   2..255: TmpString:= TmpString+ IntToStr(MyNumUser)+'-Users';
  end;
  if not RegSetValueText(HKEY_LOCAL_MACHINE, SQLMonitorSubKey, SQLMonitorSubKeyName, TmpString) then
    MessageDlg('Error registry update...',
...
var
   MyNumUser: string;
begin
   CurrentFileSize:= Form1.MyGetFileSize(MyWorkingFileName);
   Form1.StatusBar1.Panels[0].Text:= 'Reading file...';
   try
    GetMem(Buffer, CurrentFileSize);
    FlagBufferActive:= true;
    if Form1.OpenFileToMemory(MyWorkingFileName,
     ...
     begin
      ...
      Form1.StatusBar1.Panels[1].Text:=IntToStr(MyGetFileSize(WorkingFileName));
      Form1.Label2.Caption:= Form1.GetVersionSQLServer;
      MyNumUser:= StrPas(GetNumUser);
      Form1.UpdateNumUser(MyNumUser);
     end;
   except
...
if not FlagCrack then
 begin
   MessageDlg('Version of current server is not detected!!!',
              mtInformation,[mbOK],0);
 end
else
 begin
   MessageDlg('Max Number of user is seted succesful...'#13#10 +
              'Please, rename new exe-file ' + MyWorkingFileName + SuffixNewFile +#13#10+
              'to'#13#10 +
              MyWorkingFileName,
              mtInformation,[mbOK],0);
   Form1.UpdateNumUser(GetNumUser);
 end;
end;
