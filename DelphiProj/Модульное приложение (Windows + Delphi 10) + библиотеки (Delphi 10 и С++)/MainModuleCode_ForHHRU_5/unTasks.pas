...
uses
  ...
  IdUDPClient, Dialogs, IOUtils, ActiveX, Vcl.AxCtrls, IdGlobal, EncdDecd,
  unConst, unVariables;
...
//    FLineIndex_ForView: integer;
   ...
   public
    constructor Create(TaskLibraryId, TaskTemplateId, TaskNum: word; inputBeginState: TTaskState); overload;
    ...
    property LibraryId: WORD read FLibraryId;
...
//--- Поток для журнала событий от задачи
...
//--- Поток для журнала событий от ядра звдвчи
...
//------------------------------------------------------------------------------
  TTaskCore = class (TTaskItem)
   ...
   protected
    ...
    FStream_Log: TStream;
    FStream_Log_LastPos: DWORD;
    FTaskCoreStream: IStream; //--- Поток в "COM-формате" принимаем от библиотек результаты работы посредством TStreamAdapter
                              //--- и пишем в него же для передачи в объект "Задача" (TaskItem)
    FStringStream_Log: TStringStream;
    FStringStream_Log_LastPos: DWORD;
    FTaskSourceStream_Log: IStream; //--- Поток в "COM-формате" принимаем поток для журнала от библиотек посредством TStreamAdapter
    FTaskSourceStream_Log_LastPos: DWORD;  //--- это копия указателя на интерфейс потока (IStream) от источника задачи из библиотеки
                                  //--- для передачи в объект "Задача" (TaskItem). TaslItem работает только с ядром задачи (не напрямую с источником задачи)
    procedure Execute; override;
   public
    constructor Create(inputTaskItemOwner: TTaskItem); overload;
    ...
    property Stream_Log: TStream read FStream_Log write FStream_Log;
    property TaskSourceStream_Log: IStream read FTaskSourceStream_Log write FTaskSourceStream_Log;
    property StringStream_Log: TStringStream read FStringStream_Log write FStringStream_Log;
    property StringStream_Log_LastPos: DWORD read FStringStream_Log_LastPos write FStringStream_Log_LastPos;
...
//------------------------------------------------------------------------------
constructor TTaskCore.Create(inputTaskItemOwner: TTaskItem);
var
  tmpDWord: DWORD;
begin
 ...
 FTaskItemOwner:= inputTaskItemOwner; //--- Сохранили владельца (TaskItem) данного ядра задачи

//--- Создание источника задачи из библиотеки
 FTaskSource:= LibraryList[FTaskItemOwner.FLibraryId].LibraryAPI.NewTaskSource(FTaskItemOwner.FTaskTemplateId, FTaskItemOwner.FTaskNum);
 tmpDWord:= FTaskItemOwner.ThreadID;
 FTaskSource.SetOwnerThread(tmpDWord);

 FTaskItemOwner.FTaskSource:= FTaskSource;
 FTaskSource.SetTaskMainModuleIndex(FTaskItemOwner.FTaskNum);

//--- Создание потока для передачи информации для журнала в "управляющий поток задачи" - TaskItem
//--- через адаптер StringStream
//--- (ядро задачи является "рядовым участником обмена" и может только отправлять информацию (в данном случае в журнал))
//--- т.е. в ядре задачи не происходит приём-передача информации - это пеализовано в задаче (TaskItem)
//--- Запись в поток "начальных данных" текущего ядра задачи (наименование, номер)
  FStringStream_Log:= TStringStream.Create('', TEncoding.ANSI);
  FStringStream_Log_LastPos:= 0;
//  FTaskCoreStream_Log:= TStreamAdapter.Create(FStringStream_Log, soReference);
// DecodeStream(StringStream_Log, StringStream_Log);

//--- Проброс IStream от источника задачи в TaskItem (TaskItem не работает напрямую с источником задачи, только через ядро задачи)
//--- TaskItem будет использовать FTaskSourceStream_Log через TaskCore
  FTaskSourceStream_Log:= TaskSource.Stream_Log;
  FTaskSourceStream_Log_LastPos:= 0; //--- Будет отслеживаться последняя позиция, до которой TaskItem прочитал журнал источника задачи
end;
...
//------------------------------------------------------------------------------
constructor TTaskItem.Create(TaskLibraryId, TaskTemplateId, TaskNum: word; inputBeginState: TTaskState);
var
  tmpBool: boolean;
  tmpString: WideString;
  tmpDWord: DWORD;
...
//--- В TaskItem и TaskCore свои ссылки на объект TaskSource в библиотеке
 FTaskCore:= TTaskCore.Create(self);

//  self.FTaskCore.TaskItemOwner:= self;   //--- передача владельца в TaskCore  //--- уже передан в конструкторе
//--- Настроим ядро задачи на соответствующую задачу из библиотек
 FTaskCore.TaskSource.SetTaskMainModuleIndex(TaskNum);

 //--- Создание потока для передачи информации для журнала главного модуля
//--- Запись в поток "начальных данных" (наименование, номер)
//--- Подключение потока для журнала (IStream) от "источника задачи", проброшенного через "ядро задачи"
 FStream_Core_Log:= TOleStream.Create(TaskCore.TaskSourceStream_Log);
 FStream_Core_Log.Position:= 0;
//--- Создание потока информации для журнала задачи (TaskItem)
//--- в него бет записывать свои события TaskItem и подгружать из журнала ядра задачи
 tmpString:= format(wsTaskItem_Title, [FTaskNum, ThreadID]);
 StringStream_Log:= TStringStream.Create(tmpString, TEncoding.ANSI); //--- При создании ядра не пишем в лог отдельное сообщение
//--- Начальная загрузка информации для журнала от ядра задачи и источника задачи
 StringStream_Log.LoadFromStream(FStream_Core_Log); //--- Загрузка "на старте" из журнала источника задачи
...
//--- Начальное значение для потока (информация о результате выполнения от задач)
 ...
 //--- Настроим ядро задачи на соответствующую задачу из библиотек
//---------------------- Для потоков с результатами от источника задач ---------
//--- 2.2 Настройка потока передачи результатов от задач в главный модуль
//showmessage('Перед ....Stream:= TOleStream.Create(TaskList[tmpInt].TaskSource.Stream_Result); - Запуск новой задачи');
   Stream:= TOleStream.Create(TaskSource.Stream_Result);
   Stream.Position:= 0;

//--- Запускаем получение потока с результатами из задачи в библиотеке
   StringStream:= TStringStream.Create;
   StringStream.LoadFromStream(Stream);
   DecodeStream(StringStream, StringStream);

//--- присвоим состояние задачи, которое пришло на входе данной подпрограммы
 TaskState:= inputBeginState;

//  tmpString:= format(wsResultStreamTitle, [FTaskLibraryIndex, FTaskLibraryIndex]) + wsCRLF;
//   FStream_Log:= TOleStream.Create(FTaskSource.Stream_Log);
//   FStream_Log.Position:= 0;
...
//--- Обработка сообщений от ядра-задачи будет в TaskItem.Queue, а в исходнике задачи отправляется сообщение PostThreadMessage)
//--- Теперь ядро задачи напрямую отправляет оповещение в главный модуль
...
//--- Восстановление исходных значений TaskNum и SenderId из упакованного формата
     ...
     tmpInt:= (tmpMsg.WParam and LoWordMask);
     tmpWord:= ((tmpMsg.WParam and HiWordMask) shr 16);

     case tmpWord of
//--- sidTaskItem
      NativeUInt(msTaskCore):
       begin
        TaskCore.FStream_Log.Position:= TaskCore.FStream_Log_LastPos;
        StringStream_Log.LoadFromStream(TaskCore.FStream_Log);
        TaskCore.FStream_Log_LastPos:= TaskCore.FStream_Log.Position;
       end;

//--- sidTaskCore
      NativeUInt(msTaskSource):
       begin
        FStream_Core_Log.Position:= TaskCore.FTaskSourceStream_Log_LastPos;
        StringStream_Log.LoadFromStream(FStream_Core_Log);
        TaskCore.FTaskSourceStream_Log_LastPos:= FStream_Core_Log.Position;
       end;
...
//-----------------------------------------------------------------------------------------------------------------------------
       CriticalSection.Leave;
//       PostThreadMessage(self.FMainModuleOwner, WM_Data_Update, tmpMsg.wParam, CMD_SetMemoLogStreamUpd); //--- Так было до проброса
       if not PostThreadMessage(self.FMainModuleOwner, WM_Data_Update, MakeDwordAsSender(FTaskNum, DWORD(msTaskItem)), CMD_SetMemoLogStreamUpd) then
       begin
showmessage('PostThreadMessage не выполнен');
       end;
      end;
     end;
    end;
...
begin
  ...
  begin
   for tmpInt:= 0 to (TaskList.Count - 1) do
   begin
    if TaskList[tmpInt].TaskNum > Result then
     Result:= TaskList[tmpInt].TaskNum;
   end;
   inc(Result); //--- Далее работа идёт с tmpInt
  end;
