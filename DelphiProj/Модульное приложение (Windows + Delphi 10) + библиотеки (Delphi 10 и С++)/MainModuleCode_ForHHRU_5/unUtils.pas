...
//--- Настройка потока (подключение к IStream) передачи результатов из библиотек в главный модуль
  LibraryList[inputLibraryNum].Stream_Log:= TOleStream.Create(LibraryList[inputLibraryNum].LibraryAPI.Stream_Log);
  LibraryList[inputLibraryNum].StringStream.LoadFromStream(LibraryList[inputLibraryNum].Stream_Log);
  LibraryList[inputLibraryNum].Stream_Log_LastPos:= 0;
 ...
 begin
  ...
  begin
   try
    ...
    LibraryList[tmpInt].Stream_Log.Free;
