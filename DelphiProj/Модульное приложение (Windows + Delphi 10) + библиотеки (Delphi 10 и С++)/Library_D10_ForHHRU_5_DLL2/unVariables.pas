...
function TBSTRItems.GetString(var ItemIndex: DWORD): BSTR;
begin
...
//------------------------------------------------------------------------------
procedure TTaskSource.SetTaskMainModuleIndex(var inputTaskMainModuleIndex: WORD);
begin
...
try
 ...
 try
  ...
  self.WriteDataToLog(format(wsTask_AbortedOnError,
                      [self.FTaskLibraryIndex, tmpLibraryAPI.GetTaskList[tmpDWord], self.FTaskMainModuleIndex, Exception(tmpObject).ClassName +
...
//Получаем код завершения.
  ...
  try
   tmpLibraryAPI:= TLibraryAPI.Create;
   tmpDWord:= self.FTaskLibraryIndex;
   self.WriteDataToLog(format(wsTask_DoneMessage, [self.FTaskLibraryIndex, tmpLibraryAPI.GetTaskList[tmpDWord],
  ...
  begin
   tmpDWord:= self.FTaskLibraryIndex;
   self.WriteDataToLog(format(wsTask_AbortedOnError,
                       [self.FTaskLibraryIndex, tmpLibraryAPI.GetTaskList[tmpDWord], self.FTaskMainModuleIndex, tmpE.ClassName +
...
//--- Установить тип отправителя - API Библиотеки
  try
   Result:= PostThreadMessage(FOwnerThread, WM_Data_Update, MakeDwordAsSender(self.FTaskMainModuleIndex, WORD(msTaskSource)), CMD_SetMemoLogStreamUpd);
   if not Result then
   begin
