...
//----- Создание таймера - индивидуальный для каждого объекта ------------------
 FLastOSTickCount:= GetTickCount(); //--- Для начала отсчёта периода отчёта
...
//  CurrentNumber:= CurrentNumber + 1;
  if TaskItem.FTaskStepCount < 3 then
   begin
    iTestNumber:= 3;
    StreamWriter.WriteLine('Простые числа:');
   end
  else
  ...
  TaskItem.InfoFromTask:= 'Пройден ' + IntToStr(TaskItem.FTaskStepCount) + ' усл.цикл, ' + Format('найдено: %d простых чисел', [TaskItem.FTaskStepCount]);
